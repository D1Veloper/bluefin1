...
RUN dnf5 install kernel-devel kernel-headers gcc make dkms acpid libglvnd-glx libglvnd-opengl libglvnd-devel pkgconfig
RUN dnf5 install -y akmod-nvidia 
RUN echo -e "blacklist nouveau\noptions nouveau modeset=0" | sudo tee /etc/modprobe.d/blacklist-nouveau.conf

RUN dnf5 install -y tmux 
RUN dnf5 install -y htop
RUN dnf5 install -y btop
RUN yes | dnf5 install -y neofetch
RUN dnf5 clean all
